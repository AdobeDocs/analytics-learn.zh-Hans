---
title: 将产品级分段与子点击分析结合使用
description: 了解如何使用Adobe Analytics中的子点击分析实现更精确的分段和报表。
feature: Segmentation
role: User
level: Experienced
doc-type: Feature Video
last-substantial-update: 2026-08-11T00:00:00Z
jira: KT-22301
source-git-commit: c3fe1c5d91aea129e8ec7b816917ccdcb9abf876
workflow-type: tm+mt
source-wordcount: '114'
ht-degree: 6%

---

# 在Adobe Analytics中将产品级分段与子点击分析结合使用

Adobe Analytics中的传统分段通常仅限于点击级别，当多个产品、计划或对象与单个事件关联时，这可能会带来挑战。 当分析师想要隔离特定项目的性能时，这可能会导致量度夸大或归因不准确。

此视频演示了Adobe Analytics中的子点击分析如何提供更细粒度的分段功能。

有关其他信息，请访问[文档](https://experienceleague.adobe.com/zh-hans/docs/analytics/components/segmentation/sub-hit){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/3497083/?captions=chi_hans&quality=12&learn=on)
